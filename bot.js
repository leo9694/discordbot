...
const status_comandos=['ff','def','agi','ind','totf','pu','int','anal','totm','cab','caf','cam','cei','test','qtrf','qtrm','nivel']
...
if (verificar_comando(comando)==true){  
   if (verificar_comando_composto(comando)==false){       
      if(comando=='cadastrarbot'){
         ...
         if(!args[0]){ 
         ...
         }else if(resposta=='ff' || resposta=='agi' || resposta=='ind' || resposta=='def' ){                  
            let msn='```'            
            ...
            let msn='```ini\n'             
            var atr = resposta.toLowerCase().split("+")            
            let dano= parseInt(bot.status[`${atr[0]}`])+parseInt(bot.status[`${atr[1]}`])
            msn+=`Dano/Defesa [${dano}]`
            msn+='```' 
            return message.channel.send(msn)
